...
public function Main($login=1,$enc = 'UTF-8',$ignore_default_settings='no',$use_this_main = 'no') {
	global $main;
...
public function check_persistent_logins() {
	if ($_SESSION["failed_login_attempt"] > 4) {
		if ((mktime() - $_SESSION["failed_login_attempt_last_time"]) < 300) {
			return 1;//block 
		}
		else {
...
public function check_ip_location() {
	...
	$country .= ",";
	if (strpos($main["block_countries_from_ip"],$country) !== false) {
		//echo "Blocked<br>";
...
global $main,$db;
	include($this->settings["local_url"]."/layout/".$this->get_setting("admin_default_layout")."/menus.php");
	//include($this->settings["local_url"]."/layout/".$this->get_setting("admin_default_layout")."/header.php");
